...
const LikeMutationQueueContext = createContext<
  LikeMutationQueueContextType | undefined
>(undefined);

export function LikeMutationQueueProvider({
  children,
}: {
    ...
    (target: string, cid: string) => {
      ...
      if (likedRecord) {
        ...
        if (likedRecord.uri !== "pending") {
          enqueue({
            type: "unlike",
            likeRecordUri: likedRecord.uri,
            target,
            originalRecord: likedRecord,
          });
        }
      } else {
  ...
  const backfillState = async (target: string, user: string) => {
    ...
    const likes =
      (data as linksRecordsResponse)?.linking_records?.slice(0, 50) ?? [];
...
export function useLikeMutationQueue() {
  ...
  if (context === undefined) {
    throw new Error(
      "useLikeMutationQueue must be used within a LikeMutationQueueProvider",
    );
  }
  return context;
}
